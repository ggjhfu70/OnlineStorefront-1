...
import { mockEnhancedOrders } from '../data/centralizedMockData';

export const useOrders = () => {
  const [orders, setOrders] = useState<EnhancedOrder[]>(mockEnhancedOrders);
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const orderService = serviceFactory.getService('orders');
